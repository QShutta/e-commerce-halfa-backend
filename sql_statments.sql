...
-- إنشاء أو تحديث "جدول وهمي" (فيو) باسم top_selling_view
CREATE or REPLACE VIEW top_selling_view AS
...
-- بنربط جدول السلة بجدول المنتجات
-- عشان نجيب بيانات المنتج (زي اسمه وصورته) بناء على الـ ID بتاعه
INNER JOIN products ON cart.cart_product_id = products.products_id

-- دة أهم فلتر: إحنا بنجيب بس المنتجات اللي "اتباعت فعلاً" (يعني ليها رقم أوردر مش صفر)
-- وبنتجاهل اللي لسه في السلة (اللي الـ cart_order بتاعها بصفر)
WHERE cart.cart_order != 0


-- هنا السحر كله: بنجمّع كل الصفوف اللي لنفس المنتج 
-- (ليها نفس product_id)
--  في صف واحد بس
-- دة ضروري عشان الـ 
-- COUNT
--  اللي فوق تشتغل صح (تعد لكل منتج لوحده)
GROUP BY cart.cart_product_id

-- بنرتب النتيجة "تنازلياً"
--  (DESC)
--  حسب عمود الـ 
-- productCount
-- عشان المنتج الأكثر مبيعاً (اللي رقمه أعلى) يظهر في الأول
ORDER BY productCount DESC

-- بنحدد إننا عايزين نعرض "أول 6 منتجات بس" من القائمة المترتبة (يعني الـ Top 6)
LIMIT 6;